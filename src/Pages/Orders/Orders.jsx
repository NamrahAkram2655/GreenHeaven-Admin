        ...
        {orders.length > 0 ? (
          <div className="space-y-6">
            {orders.map((order, index) => (
              <motion.div
              key={index}
              className="shadow-md sm:shadow-lg rounded-md sm:rounded-lg p-4 sm:p-6 border border-green-600 flex flex-col gap-4 transition-all"
              whileHover={{ scale: 1.02 }} // A lighter hover for mobile experience
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                ...
                <p className="text-sm sm:text-lg font-extrabold text-gray-900">
                  Order #{index + 1} | {new Date(order.date).toLocaleDateString()}
                </p>
              </div>
              ...
              <div>
                {order.items.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-3 border-b pb-2 mb-2 last:border-none last:pb-0 last:mb-0"
                  >
                    <motion.img
                      src={item.image || "/placeholder.jpg"}
                      alt={item.name}
                    ...
                    />
                    <div>
                      <p className="text-green-900 font-bold text-sm sm:text-base">{item.name}</p>
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-2 sm:items-center">
                ...
                <select
                  className="px-3 py-2 text-sm bg-gray-100 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  value={order.status}
                  onChange={(e) =>
                    updateOrderStatus(order._id, e.target.value, order.amount)
                  }
